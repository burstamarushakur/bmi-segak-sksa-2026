...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==='getClassStatuses'){
   const year=Number(body.year),pengisian=Number(body.pengisian||1);const roster=await rpc('segak_bridge_year_roster',{p_year:year});if(!roster.length)return json({success:true,statuses:[]});
   // Jangan guna .in() dengan semua UUID satu sekolah: URL PostgREST jadi terlalu panjang dan status list boleh kosong.
   const [{data:profiles,error:pe},{data:records,error:re}]=await Promise.all([
    db.from('segak_student_profiles').select('student_id,gender'),
    db.from('segak_records').select('student_id,height_cm,weight_kg,step_pulse,push_ups,curl_ups,sit_reach_cm').eq('session_year',year).eq('pengisian',pengisian)
   ...
   const statuses=[...groups.entries()].map(([className,members])=>{const yl=Number(members[0]?.year_level||0);const bmiFilled=members.filter((m:any)=>{const p:any=pm.get(m.student_id),r:any=rm.get(m.student_id);return!!p?.gender&&r?.height_cm!=null&&r?.weight_kg!=null}).length;const segakFilled=yl<4?0:members.filter((m:any)=>{const p:any=pm.get(m.student_id),r:any=rm.get(m.student_id);return!!p?.gender&&[r?.step_pulse,r?.push_ups,r?.curl_ups,r?.sit_reach_cm].every(v=>v!=null)}).length;return{className,yearLevel:yl,totalStudents:members.length,bmiFilled,segakFilled,bmiComplete:members.length>0&&bmiFilled===members.length,segakComplete:yl>=4&&members.length>0&&segakFilled===members.length}});return json({success:true,statuses});
  }
  ...
  if(action==='getStudentBssr'){
   ...
   const [{data:p,error:pe},{data:records,error:re},{data:set}]=await Promise.all([db.from('segak_student_profiles').select('*').eq('student_id',studentId).maybeSingle(),db.from('segak_records').select('*').eq('student_id',studentId).gte('year_level',1).lte('year_level',6).order('session_year').order('pengisian'),db.from('segak_settings').select('value').eq('key','school_name').maybeSingle()]);if(pe)throw pe;if(re)throw re;
